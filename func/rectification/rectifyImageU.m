function [TL,TR,err] = rectifyImageU(img_base)
...
load(fullfile('bin/',img_base,[img_base '_pairs']))
...
%figure(1)
subplot(2,2,1)
[IL] = imread(fullfile('bin/',img_base,['u_' img_base '_l.bmp']),'bmp');
image(IL);
axis image
hold on
title('Left image');
...
%figure(2)
subplot(2,2,2)
[IR] = imread(fullfile('bin/',img_base,['u_' img_base '_r.bmp']),'bmp');
image(IR);
axis image
hold on
title('Right image');
...
imwrite(JL,fullfile('bin',img_base,['rectified_' img_base '_l.bmp']),'bmp');
imwrite(JR,fullfile('bin',img_base,['rectified_' img_base '_r.bmp']),'bmp');
